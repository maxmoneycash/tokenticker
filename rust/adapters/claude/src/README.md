...
- turbotokens keeps the parent entry and drops the replayed sidechain copy when at
...
- This behavior fixes the overcounting reported in
  [#913](https://github.com/turbotokens/turbotokens/issues/913).
...
- Advisor usage is stored in `message.usage.iterations` records whose type is
  `advisor_message`.
- turbotokens counts each advisor record separately under the record's own model.
