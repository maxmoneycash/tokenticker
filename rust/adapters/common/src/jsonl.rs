...
//!    [`byte_lines`](turbotokens_core::fast::byte_lines), avoiding a `String` allocation
//!    per line.
...
use turbotokens_core::fast::{LinePrefilter, byte_lines};
...
///
/// Mirrors [`turbotokens_core::non_empty_json_string`]: non-string values and
...
{
    let value = Option::<serde_json::Value>::deserialize(deserializer)?;
    Ok(turbotokens_core::non_empty_json_string(value.as_ref()))
}
...
mod tests {
    ...
    use turbotokens_core::fast::LinePrefilter;
