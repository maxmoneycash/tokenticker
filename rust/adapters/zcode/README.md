...
focused and unified reports. By default it reads `~/.zcode`; set `ZCODE_HOME`
to another ZCode data directory. Comma-separated roots are accepted for audits
across copied stores.

```bash
turbotokens zcode daily --json
turbotokens zcode weekly
turbotokens zcode monthly
turbotokens zcode session
```
...
- `report.rs` — daily, weekly, monthly, and session summary shapes

Source-specific behavior stays here; shared report and pricing behavior belongs
in `turbotokens-core` or `turbotokens-adapter-common`.
...
weekly, monthly, and session summaries use the standard turbotokens report
shapes.
...
```text
cargo test -p turbotokens-adapter-zcode
```
