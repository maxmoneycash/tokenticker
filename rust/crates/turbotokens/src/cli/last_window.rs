use turbotokens_cli::{AgentCommandArgs, AgentReportKind, Command, SharedArgs, WeekDay};
use turbotokens_core::{PeriodUnit, format_date, last_periods_since, utc_now};
...
fn window_target(cli: &mut Cli) -> Option<(&mut SharedArgs, PeriodUnit, WeekDay)> {
    match cli.command.as_mut() {
        // A bare `turbotokens` is the unified daily report.
...
/// Every report that buckets by week outside of `turbotokens claude weekly` starts
...
#[cfg(test)]
mod tests {
    use turbotokens_cli::{CodexSpeed, SortOrder, WeeklyArgs};
