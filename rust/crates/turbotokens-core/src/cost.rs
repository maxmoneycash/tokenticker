...
/// Computes the display cost and the missing-pricing marker for one usage
/// record with at most one pricing lookup. Callers processing thousands of
/// records per report use this instead of calling `calculate_cost_for_usage`
/// and `missing_pricing_model_for_usage` separately, which each resolve the
/// model's pricing on their own.
pub fn cost_and_missing_model_for_usage(
    model: Option<&str>,
    usage: crate::TokenUsageRaw,
    cost_usd: Option<f64>,
    mode: CostMode,
    ...
    let found = model.and_then(|model| pricing.and_then(|pricing| pricing.find(model)));
