...
impl DoubleEndedIterator for ByteLines<'_> {
    fn next_back(&mut self) -> Option<Self::Item> {
        ...
        // A single trailing newline terminates the final line rather than
        // delimiting an empty one, mirroring the forward direction.
        let hay = if self.bytes.len() > 1 && self.bytes.last() == Some(&b'\n') {
            &self.bytes[..self.bytes.len() - 1]
        } else {
            self.bytes
        };
        if let Some(newline) = memrchr(b'\n', hay) {
            let line = &hay[newline + 1..];
            self.bytes = &hay[..newline];
            Some(line)
        } else {
            let line = hay;
            self.bytes = &[];
...
mod tests {
    ...
    #[test]
    fn byte_lines_iterates_backwards_over_the_same_lines() {
        for content in [
            b"one\ntwo\nthree".as_slice(),
            b"one\ntwo\nthree\n".as_slice(),
            b"one".as_slice(),
        ] {
            let forward = byte_lines(content).collect::<Vec<_>>();
            let backward = byte_lines(content).rev().collect::<Vec<_>>();
            let mut reversed = forward.clone();
            reversed.reverse();
            assert_eq!(backward, reversed);
        }
        assert!(byte_lines(b"").next_back().is_none());
    }
